...
/**
 * The interface defining operations making up the Cloud Foundry Java client's
 * API.
 *
...
public interface CloudFoundryOperations {
	...
	/**
	 * Override the default REST response error handler with a custom error
	 * handler.
	 *
	 * @param errorHandler
	 */
	 ...
	 *
	 * @param orgName
	 *            name of the organization containing the space
	 * @param spaceName
	 *            name of the space
	 * @return List of space manager UUID
	 */
	 ...
	 *            name of the space
	 * @param userGuid
	 *            guid of the user. If null, use current user. To retrieve user
	 *            guid, use {@link #getOrganizationUsers(String)
	 ...
	 *
	 * @return List of CloudOrganizations objects containing the organization
	 *         info
	 */
	 ...
	 * @param required
	 *            if true, and organization is not found, throw an exception
	 * @return
	 */
	...
	/**
	 * Update the password for the logged in user using the
	 * username/old_password provided in the credentials.
	 *
	 * @param credentials
	 *            current credentials
	 * @param newPassword
	...
	/**
	 * Get application environment variables for the app with the specified
	 * name.
	 *
	 * @param appName
	 *            name of the app
	 ...
	 *            memory to use in MB
	 * @param uris
	 *            list of URIs for the app
	 * @param serviceNames
	 *            list of service names to bind to app
	 */
	 ...
	 * @param callback
	 *            a callback interface used to provide progress information or
	 *            <tt>null</tt>
	 * @throws java.io.IOException
	 */
	 ...
	 * Upload an application to Cloud Foundry.
	 *
	 * This form of <tt>uploadApplication</tt> will read the passed
	 * <tt>InputStream</tt> and copy the contents to a temporary file for
	 * upload.
	 *
	 * @param appName
	 *            the application name
	 * @param fileName
	 *            the logical name of the application file
	 * @param inputStream
	...
	/**
	 * Start application. May return starting info if the response obtained
	 * after the start request contains headers. If the response does not
	 * contain headers, null is returned instead.
	 *
	 * @param appName
	 *            name of application
	 * @return Starting info containing response headers, if headers are present
	 *         in the response. If there are no headers, return null.
	 */
	 ...
	 *
	 * @param appName
	 *            name of appplication
	 * @param services
	 *            list of services that should be bound to app
	 */
	 ...
	 *            name of appplication
	 * @param staging
	 *            staging information for the app
	 */
	 ...
	 *            name of application
	 * @param uris
	 *            list of URIs the app should use
	 */
	void updateApplicationUris(String appName, List<String> uris);

	/**
	 * Update application env using a map where the key specifies the name of
	 * the environment variable and the value the value of the environment
	 * variable..
	 *
	 * @param appName
	 *            name of application
	...
	/**
	 * Update application env using a list of strings each with one environment
	 * setting.
	 *
	 * @param appName
	 *            name of application
	...
	/**
	 * Get logs from the deployed application. The logs will be returned in a
	 * Map keyed by the path of the log file (logs/stderr.log, logs/stdout.log).
	 * 
	 * @param appName
	 *            name of the application
	 * @return a Map containing the logs. The logs will be returned with the
	 *         path to the log file used as the key and the full content of the
	 *         log file will be returned as a String value for the corresponding
	 *         key.
	 ...
	 *
	 * This method has 'tail'-like behavior. Every time there is a new log
	 * entry, it notifies the listener.
	 *
	 * @param appName
	...
	/**
	 * Get the staging log while an application is starting. A null value
	 * indicates that no further checks for staging logs should occur as staging
	...
	/**
	 * Get a the content, starting at a specific position, of a file from the
	...
	/**
	 * Get a range of content of a file from the deployed application. The range
	 * begins at the specified startPosition and extends to the character at
	 * endPosition - 1.
	 *
	 * @param appName
	 *            name of the application
	...
	/**
	 * Get a the last bytes, with length as specified, of content of a file from
	...
	/**
	 * Provide the content of a file from the deployed application via
	 * callbacks.
	 *
	 * @param appName
	 *            name of the application
	 ...
	 * @param filePath
	 *            path to the file
	 * @param clientHttpResponseCallback
	 *            callback object to receive file contents
	 */
	void openFile(String appName, int instanceIndex, String filePath,
	...
	/**
	 * Gets the default domain for the current org, which is the first shared
	 * domain.
	 *
	 ...
	 *
	 * @param host
	 *            the host of the route to register
	 * @param domainName
	 *            the domain of the route to register
	 */
	 ...
	 *
	 * @param host
	 *            the host of the route to delete
	 * @param domainName
	 *            the domain of the route to delete
	 */
	 ...
	 * <p/>
	 * This method requires the logged in user to have admin permissions in the
	 * cloud controller.
	 *
	 ...
	 * @return the CloudSecurityGroup or <code>null</code> if no security groups
	...
	/**
	 * Create a new CloudSecurityGroup using a JSON rules file. This is
	 * equivalent to
	 * <code>cf create-security-group SECURITY-GROUP PATH-TO-RULES-FILE</code>
	 * when using the cf command line. See the Application Security Group
	 ...
	 * @param jsonRulesFile
	 *            An input stream that has a single array with JSON objects
	 *            inside describing the rules
	 ...
	 *             if a security group does not exist with the name of the given
	 *             CloudSecurityGroup
	 */
	void updateSecurityGroup(CloudSecurityGroup securityGroup);

	/**
	 * Updates a existing CloudSecurityGroup using a JSON rules file. This is
	 * equivalent to
	 * <code>cf update-security-group SECURITY-GROUP PATH-TO-RULES-FILE</code>
	 * when using the cf command line. See the Application Security Group
	 * documentation for more details.
	 * <p/>
	 * Example JSON-formatted rules file:
	 ...
	 *            inside describing the rules
	 * @throws IllegalArgumentException
	 *             if a security group does not exist with the given name
	 * @see http://docs.cloudfoundry.org/adminguide/app-sec-groups.html
	 */
	...
	/**
	 * Bind a security group to the list of security groups to be used for
	...
	/**
	 * List security groups in the set of security groups for running
	 * applications.
	 * <p/>
	 * This method requires the logged in user to have admin permissions in the
	 * cloud controller.
	 ...
	 *
	 * @param orgName
	 *            The name of the organization that the space is in.
	 * @param spaceName
	 *            The name of the space
	 * @param securityGroupName
	 *            The name of the security group to bind to the space
	 * @throws IllegalArgumentException
	 *             if the org, space, or security group do not exist
	 */
